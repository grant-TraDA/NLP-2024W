...
def classify_sister_cities(text):
    try:
        result = classifier(f"Determine if the following agreement is under the patronage of Sister Cities International: {text}", labels)
...
data['Sister Cities Classification'] = data['cleaned_text'].apply(lambda x: classify_sister_cities(str(x)))
...
# Save the results
output_path = 'outputs/tasks/sister_cities_classification_results.csv'
data[["file_name","Is Sister Cities International"]].to_csv(output_path, index=False)
