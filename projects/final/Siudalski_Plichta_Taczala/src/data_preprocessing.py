...
def extract_and_clean_text(htm_content):
    ...
    raw_text = re.sub(
        r"\b\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b", "", raw_text
    )  # Phone numbers
    raw_text = re.sub(
        r"\bP\.?O\.?\s?Box\b.*?\d+", "", raw_text, flags=re.IGNORECASE
    )  # PO Box
    raw_text = re.sub(
        r"[^\w\s,.]", "", raw_text
    )  # Retain alphanumeric characters, spaces, commas, and periods
...
for state_folder in sorted(os.listdir(base_directory)):
    state_path = os.path.join(base_directory, state_folder)

    if os.path.isdir(state_path):  # Check if it's a folder
        state_files = sorted(os.listdir(state_path))

        for file in tqdm(state_files, total=len(state_files)):
            file_path = os.path.join(state_path, file)
            file_name, file_extension = os.path.splitext(file)

            if file_extension == ".htm":
                if file_name + ".pdf" in state_files:
                    continue
                continue
        ...
        print(f"Processed: {state_folder}")
...
df.to_csv(
    os.path.join(output_dir, "preprocessed_agreements_all_states.csv"), index=False
)
